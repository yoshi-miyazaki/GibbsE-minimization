...
void massBalance::create_massBalance(MoleculeData_G& list){
    ...
    /* show whether i'th element is used in the system or not 
     BUT we are now considering oxides, and we are EXCLUDING O2 from the system

     This applies when all Fe exists as 2+.
     When 3+ exists, usual nullspace should be used !!! */
    
    /* check which elements are used in the system 
       here we do NOT count O (oxygen), since it is just an attached element to the others */
    ElementData elementdatabase;
    int nO2 = elementdatabase.correspond("O").getNumber() - 1; /* -1 b/c C++ starts from 0 */
    for (int i=0; i<numofElement; i++){
	    for (int j=0; j<m; j++){
	        if(list[j].getComposition(i) != 0 && i != nO2){check[i] = 1;}
        }
    }
